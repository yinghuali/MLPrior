...
def get_mixture_missing_data(path, n_data, ratio, label_col_name, save_path):
    df = pd.read_csv(path)
    ...
    for s in range(n_data):
        df_tmp = df.copy()
        for i in range(n):
            cols_name = random.sample(cols_list, 1)[0]
            idx = random.randint(0, len(df))
            df_tmp[cols_name][idx]=0
...
if __name__ == '__main__':
    get_mixture_missing_data('./data/adult.csv', 10, 0.3, 'income', './data/missing/adult_mixture_missing')
    get_mixture_missing_data('./data/bank.csv', 10, 0.3, 'y', './data/missing/bank_mixture_missing')
    get_mixture_missing_data('./data/stroke.csv', 10, 0.3, 'stroke', './data/missing/stroke_mixture_missing')
